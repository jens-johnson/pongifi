 ...
 *
 * █████████████████████████████████████████████████████████████████████████████████████████████████████████████████████
 * █████████████████████████ #server/api/leagues/[leagueId]/games/[gameId]/answer.post.test.ts █████████████████████████
 *
 * Unit tests for the confirm, dispute and void route: what it writes, what it refuses, and what it returns rather than
 * throws.
 *
 * █████████████████████████████████████████████████████████████████████████████████████████████████████████████████████
 */
...
/**
 * The action double, hoisted so the module mock can consume it
 * @internal
 * @constant
 */
const answerResultMock: Mock<(...args: unknown[]) => Promise<TResultOutcome>> = vi.hoisted(
...
/**
 * The same-origin check, which a cross-site press fails
 * @internal
 * @constant
 */
...
/**
 * The operation this press belongs to
 * @internal
 * @constant
 */
 ...
 * @constant
 */
const { default: handler }: { default: EventHandler } = await import('./answer.post');

/**
 * What a committed press recorded
 * @internal
 * @constant
 */
...
describe(getTestFileName(import.meta.url), (): void => {
  beforeEach((): void => {
    ...
    answerResultMock.mockResolvedValue(EFFECT);
    readBodyMock.mockResolvedValue(buildBody());
  });

  it('answers with what the press did and where the match stands now', async (): Promise<void> => {
  ...
  it('refuses a cross-site press without parsing its body', async (): Promise<void> => {
  ...
  it('refuses a body this page does not send', async (): Promise<void> => {
    ...
    expect(answerResultMock).not.toHaveBeenCalled();
  });
  ...
  it('resolves and answers under one transaction', async (): Promise<void> => {
    // Two would let the match this caller resolved be superseded before the press landed on it
    await handler(buildEvent());
    ...
    expect(answerResultMock).toHaveBeenCalledWith(expect.anything(), USER_ID, {
      action: ResultAction.CONFIRM,
      canonicalMatchId: MATCH_ID,
      clientOperationId: OPERATION_ID,
      expectedRevision: 1,
  ...
  it('returns a conflict as a body rather than throwing it away', async (): Promise<void> => {
    ...
    answerResultMock.mockResolvedValue({
      ok: false,
  ...
  it('carries a dispute’s words through to the service', async (): Promise<void> => {
    readBodyMock.mockResolvedValue(buildBody({ action: ResultAction.DISPUTE, note: 'that was not the score' }));

    await handler(buildEvent());

    expect(answerResultMock).toHaveBeenCalledWith(
      expect.anything(),
      USER_ID,
      expect.objectContaining({ action: ResultAction.DISPUTE, note: 'that was not the score' }),
    );
  });
});
