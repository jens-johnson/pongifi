 ...
 *
 * █████████████████████████████████████████████████████████████████████████████████████████████████████████████████████
 * ███████████████████████████████████████ #server/api/me/complete.post.test.ts ████████████████████████████████████████
 *
 * Unit tests for the welcome completion endpoint's write boundary and session refresh.
 *
 * █████████████████████████████████████████████████████████████████████████████████████████████████████████████████████
 */
...
interface IProfileMocks {
  ...
  completeProfile: Mock<(userId: string, displayName: string) => Promise<IProfile | null>>;
}
 ...
 * @constant
 */
const profileMocks: IProfileMocks = vi.hoisted((): IProfileMocks => ({ completeProfile: vi.fn() }));
 ...
 * @constant
 */
const { default: handler }: { default: EventHandler } = await import('./complete.post');
...
describe(getTestFileName(import.meta.url), (): void => {
  beforeEach((): void => {
    ...
    profileMocks.completeProfile.mockResolvedValue(SAVED);
  ...
  it('saves an accepted name and hands the new session back to the account menu', async (): Promise<void> => {
    ...
    expect(profileMocks.completeProfile).toHaveBeenCalledWith(USER_ID, CHOSEN_NAME);
  ...
  it('ends the session when the account behind it has gone', async (): Promise<void> => {
    profileMocks.completeProfile.mockResolvedValue(null);
  ...
  it('finishes the welcome step, which is the one thing this endpoint exists to do', async (): Promise<void> => {
    profileMocks.completeProfile.mockResolvedValue({ ...SAVED, profileCompletedAt: null });

    await handler(EVENT);

    // The session says finished even where the stored timestamp lags, because reaching here is what finishes it
    expect(replaceUserSessionMock).toHaveBeenCalledWith(
      EVENT,
      expect.objectContaining({ user: expect.objectContaining({ needsWelcome: false }) }),
    );
  });
});
